...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        while ((loopNode = nodesLoaded.poll()) != null) {
            ...
            // Make sure it not already processed.
            if (loopNode.isDead) {
                loopNode.lock.unlock();
                continue;   // loopNode is already merged
            }
            
            // No more nodes or the node is already acquired by another thread.
            if (!loopNode.lock.tryLock()) {
                continue;
            }

            // Get the cost of the edge
            Edge<ParComponent> edge = loopNode.getMinEdge();
            if (edge == null) {
                loopNode.lock.unlock();
                solution.setSolution(loopNode);
                break;  // Single node left
            }

            // Get the other node
            ParComponent other = edge.getOther(loopNode);
            if (!other.lock.tryLock()) {
                loopNode.lock.unlock();     // Unlock the first Node
    ...
    public static final class ParComponent extends Component<ParComponent> {
       ...
       /**
        *  A lock object field for the class.
        */
